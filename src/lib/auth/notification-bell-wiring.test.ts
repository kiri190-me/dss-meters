 ...
 * 알림은 이렇게 흐른다:
 *
 *   다른 시스템들(A/S · 개선요청 · PO …)이 저마다 가진 알림
 ...
 *     않으려면 그 자리여야 한다). 🔴 이 저장소에서는 「끝에 두는 것」만으로
 *     모자라 `ml-auto` 가 함께 간다 — 폰에서 오른쪽 묶음이 `w-full` 이고
 *     오른쪽 정렬이 꺼져 있기 때문이다(AppHeader.tsx 의 폭 셈).
 *
 * ── 왜 어떤 것은 파일의 글자를 읽는가 ───────────────────────────────────
 * 레이아웃과 머리말은 실제로 불러 볼 수 없다(세션·DB·요청 맥락이 있어야 한다).
 * 그래서 그 자리들은 **구조**를 못 박는다. 반대로 판단이 들어 있는 것(거절을
 * 삼키는 일, 답을 고르는 일, 빈 목록일 때 그리지 않는 일)은 **실제로 불러
 * 본다.** 🔴 이 시험은 DB 에도 바깥 망에도 닿지 않는다.
 * ============================================================================
 */
...
const internalLayout = withoutComments(read("src/app/(internal)/layout.tsx"));
...
test("🔴 포털에 닿지 못해도 던지지 않는다 — 빈 목록과 degraded 로 돌아온다", async () => {
  ...
  process.env.SSO_CLIENT_ID = "dss-meters";
...
test("🔴 묻는 열쇠는 검증된 세션의 authSub 다 — 클라이언트가 보낸 값이 아니다", () => {
  assert.match(internalLayout, /const user = await requireSession\(\);/);
  assert.match(internalLayout, /<PortalNotificationBell subject=\{user\.authSub\} \/>/);
});

test("🔴 <Suspense> 가 감싼다 — 포털이 느려도 모든 화면 이동이 느려지지 않는다", () => {
  const bellAt = internalLayout.indexOf("<PortalNotificationBell");
  const suspenseAt = internalLayout.indexOf("<Suspense fallback={null}>");
  assert.ok(suspenseAt > 0, "🔴 종이 <Suspense> 밖에 있다 — 머리말이 포털을 기다린다");
  assert.ok(suspenseAt < bellAt, "감싸는 차례가 뒤집혔다");
  assert.match(internalLayout, /notificationBell=\{/, "머리말에 내려보내지 않는다");
});
...
test("🔴 종은 줄의 **맨 오른쪽 끝**이다 — 아니면 폰에서 펼친 목록이 잘린다", () => {
  ...
  const logoutAt = appHeader.indexOf("t.nav.logout");
  const bellAt = appHeader.indexOf("{notificationBell}");
  assert.ok(bellAt > 0, "머리말이 종을 그리지 않는다");
  assert.ok(logoutAt > 0, "로그아웃 단추를 찾지 못했다");
...
test("🔴 폰에서는 `ml-auto` 가 함께 있어야 종이 진짜 오른쪽 끝이다", () => {
  // 이 저장소의 오른쪽 묶음은 폰(<640px)에서 `w-full` 이고 오른쪽 정렬을
  // **끈다**(2026-09-18 사용자 폰 사진). 그래서 마크업에서 끝에 두는 것만으로는
  // 종이 글자 덩이를 따라 왼쪽에 붙고, 관리자·일본어에서는 묶음이 제 안에서
  // 줄을 바꿔 종이 둘째 줄 맨 왼쪽에 선다 — 그때 목록은 거의 통째로 화면
  // 밖이다. 이 두 줄이 그 짝이고, 하나라도 바뀌면 여기서 걸린다.
  assert.match(
    appHeader,
    /className="ml-auto flex w-full flex-wrap items-center gap-3 sm:w-auto sm:justify-end"/,
    "오른쪽 묶음의 폭·정렬이 바뀌었다 — 종의 ml-auto 가 아직 맞는지 다시 셈해야 한다",
  );
  assert.match(
    bellComponent,
    /<NotificationBell[\s\S]*?className="ml-auto"[\s\S]*?\/>/,
    "🔴 종에 ml-auto 가 없다 — 폰에서 펼친 목록의 왼쪽이 화면 밖으로 잘린다",
  );
});
...
test("받은 알림은 받은 차례 그대로, 받은 주소 그대로 그려진다", () => {
  ...
  const feed = normalizePortalNotificationFeed({
    items: [
      ...
      row({ key: "b", href: "http://192.168.1.132:3200/improvement-requests/2" }),
    ],
    count: 2,
  });
  ...
  assert.deepEqual(found, [
    ...
    "http://192.168.1.132:3200/improvement-requests/2",
  ]);
});
