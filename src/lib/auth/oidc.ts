 ...
 * 그 결과(ID 토큰)를 검증해 자기 세션(meters_session)을 발급할 뿐이다.
 */
import {
  createHash,
  createHmac,
...
/** 포털의 dss_sso 와도, 이 사이트의 meters_session 과도 겹치지 않는 이름. */
export const SSO_TX_COOKIE = "meters_sso_tx";
 ...
 * next build 가 환경변수 없이 돌지 못한다.
 */
let jwksCache: ReturnType<typeof createRemoteJWKSet> | null = null;
function jwks() {
  jwksCache ??= createRemoteJWKSet(
    new URL(`${env.ssoIssuer}/.well-known/jwks.json`),
  );
  return jwksCache;
}
 ...
 *
 * 🔴 여기에 **이 사이트(계측기)의 알림은 없다.** 포털은 부른 사이트 자신에게는
 *    묻지 않는다(그쪽 문서 「빠져 있는 것」 절 — 되돌기와 30초 캐시 때문이다).
 *    이 사이트에는 애초에 자체 알림이 없다(2026-09-21 확인 — 교정 기한 알림은
 *    **메일로만** 나간다. lib/mail/notify-mail.ts · docs/NOTIFY.md. 화면에
 *    쌓이는 종이 없어서 포털에 내줄 것도 없다). 그래서 받은 것이 곧 전부다.
 *    자체 알림이 생기면 그때는 `[...내것, ...받은것]` 으로 이어 붙이고 개수도
 *    **더해야** 한다.
 *
