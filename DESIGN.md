...
- 세션 없이 보호된 주소에 접근하면 **포털로 곧장** 보낸다
  (`/api/auth/sso/start?returnTo=...`). 로그인 화면은 거치지 않는다 — 2-5 참고.
...
```
┌────────────────────────────────────────────────────────────────────┐
│ 계측기 관리        [한국어 ▾] 이남준 님 [통합 로그인으로] [로그아웃]      │
├────────────────────────────────────────────────────────────────────┤
...
세션 없이 사내 구간을 열면 **로그인 화면을 거치지 않고** 포털로 곧장 간다.

```
(세션 없이) /  ──▶  /api/auth/sso/start
...
**왜 로그인 화면을 없앴나.** 이 사이트에는 자체 로그인이 없어서 그 화면에 있는
것이라고는 "포털로 가세요" 버튼 하나뿐이다. 포털 앱 런처에서 타일을 눌러 들어온
사람은 방금 포털에서 왔는데 포털로 가라는 화면을 다시 보고, 그 버튼을 눌러도 이미
로그인된 포털을 그대로 통과해 돌아온다. 아무것도 묻지 않는 화면이면 보여줄 이유가
없다. (판정은 `lib/auth/guards.ts` 의 `requireSession` 한 곳에서 한다)

**`/login` 은 남아 있다** — 로그인이 **거절됐을 때** 이유를 보여줄 자리다.
`expired` · `unknown_role` · `inactive` · `deleted` · 그 외마다 문구가 다르다.
여기서는 자동으로 다시 보내지 않는다. 그러면 무한 왕복이 된다.
...
머리말 오른쪽에 **「통합 로그인으로」** 버튼이 있다. 포털의 앱 런처(`/apps`)로 간다.

로그아웃과 나란히 있지만 생김새를 다르게 두었다 — 둘 다 이 사이트를 떠나지만,
이쪽은 세션을 그대로 두고 다녀오는 것이고 저쪽은 모든 시스템에서 나가는 것이다.
같아 보이면 잘못 누른다.
